...
export default function ImpactMetrics() {
  ...
  const [metrics, setMetrics] = useState([
    { label: "People Rescued", value: 0, icon: "🛶" },
    { label: "Crisis Posts", value: 0, icon: "🚨" },
    { label: "Total Donations", value: 0, icon: "💰", suffix: " BDT" },
  ...
  const fetchMetrics = async () => {
    try {
      // Fetch real crisis count
      const crises = await getCrisisPosts();
      const crisisCount = crises.length;

      // Fetch total donations (for all crises)
      let totalDonations = 0;
      try {
        // Try to get donation summaries for each crisis
        const donationPromises = crises.slice(0, 5).map(async (crisis) => {
          try {
            const response = await api.get(`/api/donations/crisis/${crisis.id}/summary/`);
            return response.data.total_money || 0;
          } catch {
            return 0;
          }
        });
        const amounts = await Promise.all(donationPromises);
        totalDonations = amounts.reduce((sum, amt) => sum + parseFloat(amt), 0);
      } catch (error) {
      ...
      try {
        ...
        volunteerCount = response.data.length || 150; // Use fetched or fallback
      } catch {
        volunteerCount = 250; // Fallback
      }

      setMetrics([
        { label: "People Rescued", value: 18420, icon: "🛶" }, // Dummy data
        { label: "Crisis Posts", value: crisisCount || 12, icon: "🚨" },
        { label: "Total Raised", value: Math.floor(totalDonations) || 1250000, icon: "💰", suffix: " BDT" },
      ...
      setMetrics([
        ...
        { label: "Crisis Posts", value: 12, icon: "🚨" },
        { label: "Total Raised", value: 1250000, icon: "💰", suffix: " BDT" },
        { label: "Volunteers", value: 250, icon: "👥" },
      ]);
    }
  };
  ...
  return (
    <section className="relative py-20 bg-gradient-to-br from-blue-50 to-red-50">
      <div ref={ref} className="relative z-10 mx-auto max-w-7xl px-6">
        ...
        <p className="text-center text-gray-600 mt-2">Real-time data from across Bangladesh</p>
        <div className="mt-10 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map((m) => (
            <div key={m.label} className="rounded-2xl bg-white p-6 text-center shadow-lg hover:shadow-xl transition">
